...
- [2025/12/20 21:25 v0.41] 檔案: `src/components/DiveView.tsx`, `app/page.tsx` | 內容: 放寬每日解鎖判定邏輯，只要當日有任何發文或回覆即視為已解鎖；強化 `DiveView` 視覺隔離，未解鎖前隱藏他人留言清單，並引導使用者先進行「換氣」；修正 `DiveModal` 組件介面與 Prop 傳遞 | 原因: 符合企劃書對每日互動解鎖的定義，並增加未解鎖前的功能限制與引導性
