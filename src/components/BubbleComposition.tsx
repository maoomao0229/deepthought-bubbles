...
export const BubbleComposition: React.FC<Props> = ({ originalCount, replyCount }) => {
    ...
    const width = 300, height = 150, cx = width / 2, cy = height - 20, radius = 80, strokeWidth = 35;
    ...
    return (
        <div className="bg-slate-800/50 backdrop-blur rounded-xl border border-slate-500/30 w-full min-h-[160px] flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible mt-4">
                {/* 原創字數弧線 (綠色) */}
                <path
                    d={path1}
                    fill="none"
                ...
                {/* 中心標籤 */}
                <text x={cx} y={cy - 10} textAnchor="middle" className="fill-slate-400 text-[10px] uppercase">Total</text>
                <text x={cx} y={cy - 25} textAnchor="middle" className="fill-white text-xl font-bold font-mono">{total}</text>

                {/* 圖例 */}
                <g transform={`translate(${cx - 60}, ${cy + 5})`}>
                    <circle cx="0" cy="0" r="4" fill="#54B39E" />
                    <text x="8" y="4" className="fill-slate-400 text-[8px]">原創 {originalCount}</text>
                </g>
                <g transform={`translate(${cx + 20}, ${cy + 5})`}>
                    <circle cx="0" cy="0" r="4" fill="#6571BC" />
                    <text x="8" y="4" className="fill-slate-400 text-[8px]">回覆 {replyCount}</text>
                </g>
            </svg>
        </div>
    );
